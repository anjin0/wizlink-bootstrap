...
운영 서버(Rocky Linux / RHEL 계열)에서 WizLink를 설치·업그레이드·롤백하기 위한 **배포 진입 스크립트**입니다.
공개 저장소에 있는 **암호화된 인증정보**를 내려받아 **제품키로 복호화**한 뒤, 이를 이용해
GHCR(GitHub Container Registry) 로그인과 `wizlink-deploy` **Release 번들 다운로드·검증·실행**까지
한 번에 수행합니다.
...
| 최종 동작 | 검증된 번들의 `install.sh`, `upgrade.sh`, `rollback.sh` 중 선택 실행 |
...
### 실행 방법
```bash
sudo bash ./boot.sh                         # 최초 설치(latest)
sudo bash ./boot.sh install v1.2.3          # 지정 버전 최초 설치
sudo bash ./boot.sh upgrade v1.3.0          # 지정 버전 업그레이드
sudo bash ./boot.sh rollback v1.2.3         # 호환 가능한 이전 버전 롤백
# 실행 후: 사이트 식별자 → 제품키 순서로 입력
```

인자 없는 실행은 기존 사용법과 호환되도록 `install`로 처리합니다. `upgrade`와 `rollback`은
반드시 `vX.Y.Z` 형식의 목표 tag를 명시해야 하며 `latest`를 허용하지 않습니다.
...
| `RELEASE_TAG` | `latest` | `install` 전용 기본 tag. CLI에 tag를 지정하면 CLI가 우선 |
| `GHCR_USER` | `anjin0` | GHCR 로그인 사용자 |
| `INSTALL_DIR` | `/opt/wizlink/releases` | version별 Release 보관 루트 |
...
```mermaid
flowchart TD
    A[명령과 목표 tag 검증] --> B[기존 runtime 상태와 버전 확인]
    B --> C[대화형 입력<br/>사이트 식별자 + 제품키]
    C --> D[공개 저장소에서<br/>.ghcr.enc / .deploy.enc 다운로드]
    D --> E[제품키로 openssl 복호화<br/>GHCR_TOKEN / DEPLOY_TOKEN]
    E --> F{SKIP_DOCKER=1?}
    F -- 아니오 --> G["[1/4] GHCR 로그인"]
    F -- 예 --> G2[로그인 생략]
    G --> H
    G2 --> H{SKIP_RELEASE=1?}
    H -- 예 --> Z2[검증 완료 종료]
    H -- 아니오 --> I["[2/4] Release 조회<br/>tag SemVer 검증"]
    I --> J[version별 디렉터리에 자산 다운로드]
    J --> K["[3/4] manifest + checksum + archive 검증"]
    K --> L[tar 압축 해제<br/>선택 script · VERSION 확인]
    L --> M{실행 모드}
    M -- install --> N[install.sh 실행]
    M -- upgrade --> O[upgrade.sh 실행]
    M -- rollback --> P[rollback.sh 실행]
    N --> Z["[4/4] 완료 요약"]
    O --> Z
    P --> Z
```
...
2. **릴리스 확인** (tag 조회)
3. **배포 파일 다운로드 및 검증**
4. **WizLink 서비스 설치·업그레이드·롤백**

---

## 4. 단계별 상세

### 4.1 명령·상태 점검과 대화형 입력
- `install [vX.Y.Z]`, `upgrade vX.Y.Z`, `rollback vX.Y.Z`를 해석하고 목표 tag를 검증합니다.
- 기존 `/opt/wizlink/.env`가 있으면 동일 버전 `install` 재실행만 허용하여 중단된 최초 설치를
  복구할 수 있게 합니다. `.env` 생성 후 version 기록 전에 중단된 경우도 복구 대상으로
  허용하며, `install`을 통한 version 변경은 거부하고 `upgrade` 사용을 안내합니다.
- `upgrade`와 `rollback`은 기존 production runtime과 현재 `WIZLINK_VERSION`을 요구하며,
  명백히 잘못된 버전 방향을 인증정보 입력 전에 거부합니다.
...
- 자산과 해제된 bundle은 임시 version staging 경로에서 모두 검증한 뒤
  `/opt/wizlink/releases/<VERSION>/` 보관본과 Linux `renameat2(RENAME_EXCHANGE)`로 원자 교체합니다.
  `INSTALL_DIR`을 지정하면 해당 경로를 보관 루트로 사용합니다.
...
flowchart LR
    ...
    E --> F[tar 압축 해제]
    F --> G[선택한 배포 script 실행권한<br/>VERSION 일치 확인]
```
...
### 4.7 압축 해제와 모드별 실행 — `[4/4]`
- 동일 버전의 기존 번들이 있으면 검증된 archive로 교체합니다.
- 압축 해제 후 선택한 script가 실행 가능한지, 내부 `VERSION` 파일이 tag와 일치하는지 확인합니다.
- 비밀정보가 없는 출처 메타데이터(`.release-info`: repo/tag/base_string/mode)만 기록합니다.
- `DEPLOY_TOKEN`을 해제한 뒤 다음 계약으로 실행합니다.

```bash
<BUNDLE_ROOT>/install.sh --prod --home-dir "$WIZLINK_HOME" --version "$VERSION"
<BUNDLE_ROOT>/upgrade.sh "$VERSION" --home-dir "$WIZLINK_HOME"
<BUNDLE_ROOT>/rollback.sh "$VERSION" --home-dir "$WIZLINK_HOME"
```

`rollback.sh`는 DB를 downgrade하거나 복원하지 않습니다. 현재 DB가 목표 이전 버전과 정확히
호환되는 경우에만 애플리케이션과 Wizcollector를 전환합니다.

### 4.8 요약 출력
완료 시 실행 모드, 목표 버전, 변경 전 버전, runtime과 Release 경로, 사이트 식별자를 출력합니다.
...
| `parse_cli` | 실행 모드와 목표 Release tag 검증 |
| `check_runtime_state` | 기존 production runtime과 버전 방향 사전 확인 |
| `semver_compare` | 표준 라이브러리만으로 SemVer 비교 |
| `activate_release_dir` | 검증된 staging과 기존 version 보관본을 원자 교체 |
...
실제 GitHub, GHCR와 운영 runtime을 변경하지 않고 세 모드의 인증·Release 검증·호출 계약과
버전 방향 안전장치를 확인할 수 있습니다.

```bash
bash test-boot-modes.sh
```
